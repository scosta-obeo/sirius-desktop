...
* <span class="label label-info">Modified</span> When a semantic modification is made to a diagram (e.g. Drag and Drop of an element), the notes and texts attached to the element remain attached, and the notes and texts contained in the element are preserved as far as possible.
* <span class="label label-info">Modified</span> When an element is deleted after drag and drop, its notes and attached text are also removed if preference "Remove/hide note when the annotated element is removed/hidden" is enabled.
